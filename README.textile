...
If you want to avoid using the @ngsquery.pl@ script or other external LIMS systems, you can do so by creating an @external_data.yml@ file and placing it in the flowcell's root directory. 
...
*NOTE:* @SampleMultiplex.csv@ is a work around for our current LIMS system not being able to handle sample-specific information like barcodes. A new LIMS system is in development which will render @SampleMultiplex.csv@ *obsolete*. Documentation will be modified once the new system is live, however most of the functionality for using the new LIMS system can be found in @lib/illuminati/external_data_lims_new.rb@. Future work includes being able to specify which external data source should be used through configuration.
